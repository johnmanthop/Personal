...
/*
void IM_draw_bitmap(int xs, int ys, int scale, const unsigned int bmp[18][15])
{
	int width = 15;
	int height = 18;

	int xe = xs + width * scale;
	int ye = ys + height * scale;

	set_draw_window(xs, ys, xe, ye);
	IM_GPIO_ResetPin(CSX_PORT, CSX_PIN);
	...
	for (int h = 0; h < height; ++h)
	{
		...
		{
			unsigned char current = bmp[h][w];

			lcd_write_data(current >> 8);
			lcd_write_data(current & 0xFF);
		}
	}
	IM_GPIO_SetPin(CSX_PORT, CSX_PIN);
}*/
